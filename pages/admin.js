...
function checkAuthAndInit() {
    ...
    console.log('Admin token found, initializing dashboard');
    // Store token for API calls
    window.adminToken = adminToken;
    
    // Valid login, set current user (will be fetched from API)
    currentUser = 'pizzaladmin';
...
function loadDashboardData() {
    ...
    // Platform statistics (placeholder data for now)
    const platformStats = {
        totalRevenue: '$14.59',
        totalBusinesses: '1',
        vaultBalance: '$0.195',
    ...
    const elements = {
        ...
        'total-businesses': platformStats.totalBusinesses,
        'vault-balance': platformStats.vaultBalance,
        'daily-transactions': platformStats.dailyTransactions,
        'total-partners': platformStats.totalPartners,
        'total-meals': platformStats.totalMeals
    };
    ...
    updateProgressBar('partner-growth', 10); // 1 out of 10 target businesses
    updateProgressBar('meal-distribution', 0.1); // Just started
    updateProgressBar('community-engagement', 25); // Early adoption
    ...
    if (currentView === 'locations') {
        loadLocationsTable();
    } else if (currentView === 'businesses') {
        loadBusinessesData();
...
function loadBusinessesData() {
    ...
    // Update CN business metrics (placeholder data)
    const cnMetrics = {
        count: 1,
        stakingEnabled: 0
    };
    
    const cnCountElement = document.getElementById('cn-count');
    const cnStakingElement = document.getElementById('cn-staking');
    
    if (cnCountElement) cnCountElement.textContent = cnMetrics.count;
    if (cnStakingElement) cnStakingElement.textContent = cnMetrics.stakingEnabled;
}
...
function loadVaultData() {
    ...
    // Update vault statistics (placeholder data)
    const vaultStats = {
        totalContributions: '$0.20',
        rewardsDistributed: '$4.80',
        giftCardFunding: '$0',
        vaultSurplus: '$0.20'
    };
    
    const elements = {
        'total-contributions': vaultStats.totalContributions,
        'rewards-distributed': vaultStats.rewardsDistributed,
        'gift-card-funding': vaultStats.giftCardFunding,
        'vault-surplus-stat': vaultStats.vaultSurplus
    };
...
function loadTransactionsData() {
    ...
    // Update transaction feed with recent transactions
    const transactionFeed = document.getElementById('transaction-feed');
    if (transactionFeed) {
        // For now, keep the existing placeholder transaction
...
function loadSystemHealthData() {
    ...
    // System health indicators are static for now
    // All systems showing as operational with green status
}

// Refresh data function (called by quick action button)
